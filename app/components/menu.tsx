...
export default function Menu() {
  ...
  return (
    <motion.section
      id="menu"  // ← add this
      className="w-full py-20 px-6 sm:px-16 bg-zinc-950 text-white flex flex-col items-center"
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: false, amount: 0.3 }}
      transition={{ duration: 0.8, ease: "easeOut" }}
    >
      <h2 className="text-4xl sm:text-5xl font-bold text-center mb-2 text-[#ff5a5f]">
        Our Menu
      </h2>
      ...
      <div className="relative w-full max-w-6xl overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            {slides[current].map((pizza, i) => (
              ...
              >
                {/* Inner wrapper allows glow overflow */}
        ...
        <div className="flex justify-between mt-6 max-w-6xl w-full">
          <button
            onClick={prev}
            className="px-4 py-2 bg-zinc-800 rounded-full hover:bg-[#CEAE7B]/80 transition-colors"
          >
            Prev
          </button>
          <button
